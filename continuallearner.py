...
class ContinualLearner:
    ...
    def __init__(
        ...
        self.raw_train_envs = prepare_base_envs(
            task_names,
            benchmark=ML3(),
            # 'test', # we train on the test set of ML3 for bicameral
        ...
        self.env_id_to_name = {
            (i+1): task for i, task in enumerate(self.task_names)}
        ...
        self.agent, self.left_init_args, self.right_init_args = self.init_agent(
            self.args)
        if self.args.algorithm == 'random':
            ...
            self.storage = BiHemOnlineStorage(
                ...
                # normalise rewards for policy - set to true, but implement
    ...
    def init_agent(self, args):
        ...
        if self.args.algorithm != 'right_only':
            ...
            left_policy_net = Policy(
                ...
                # to add done flag
                dim_state=self.envs.observation_space.shape[0]+1,
            ...
            left_encoder_net = RNNEncoder(
                ...
                # for done flag
                state_dim=self.envs.observation_space.shape[0]+1,
        ...
        if self.args.algorithm != 'left_only':
            ...
            right_policy_net = torch.load(
                policy_loc, map_location=device).to(device)
            right_encoder_net = torch.load(
                encoder_loc, map_location=device).to(device)
            # freeze these!
        ...
        if self.args.algorithm == 'bicameral':
            ...
            raise NotImplementedError(
                "Set algorithm to one of left_only, right_only, bicameral or random")
        ...
        while self.envs.get_env_attr('cur_step') < self.envs.get_env_attr('steps_limit'):
            ...
            if self.args.algorithm != 'random':
                with torch.no_grad():
                    ...
                    latent, hidden_state = self.agent.get_prior(
                        self.num_processes)
                    # assert len(self.storage.latent) == 0  # make sure we emptied buffers

                    if self.args.algorithm == 'bicameral':
                        assert (len(self.storage.left_latent) == 0) & (
                            len(self.storage.right_latent) == 0)

                        self.storage.gate_hidden_states[:1].copy_(
                            hidden_state[0])
                        self.storage.left_hidden_states[:1].copy_(
                            hidden_state[1])
                        self.storage.right_hidden_states[:1].copy_(
                            hidden_state[2])
                        ...
                        assert len(self.storage.latent) == 0
            ...
            while not all(done):
                ...
                next_obs, (rew_raw, rew_normalised), done, info = self.envs.step(
                    action)
                assert all(done) == any(
                    done), "Metaworld envs should all end simultaneously"
                ...
                masks_done = torch.FloatTensor(
                    [[0.0] if _done else [1.0] for _done in done]).to(device)
                ...
                if self.args.algorithm != 'random':
                    ...
                    if self.args.algorithm != 'bicameral':
                        ...
                        self.storage.insert(
                            ...
                            rewards_raw=rew_raw.squeeze(0),
                            rewards_normalised=rew_normalised.squeeze(0),
                            value_preds=(value.squeeze(0), left_value.squeeze(
                                0), right_value.squeeze(0)),
                            masks=masks_done.squeeze(0),
                            done=torch.from_numpy(done)[:, None].float(),
            ...
            self.logger.add_tensorboard(
                'losses/value_loss', value_loss_epoch, frames)
            self.logger.add_tensorboard(
                'losses/action_loss', action_loss_epoch, frames)
            self.logger.add_tensorboard(
                'losses/entropy_loss', dist_entropy_epoch, frames)
            self.logger.add_tensorboard(
                'losses/gating_penalty', gating_penalty_epoch, frames)
            self.logger.add_tensorboard(
                'losses/total_loss', loss_epoch, frames)
            ...
            self.logger.add_tensorboard(
                'train_results/episode_rewards', task_rewards.mean(), frames)
            self.logger.add_tensorboard(
                'train_results/episode_success', task_successes, frames)
            self.logger.add_tensorboard(
    ...
    def evaluate(self, current_task, frames, eval_run):
        ...
        test_envs = prepare_parallel_envs(
            ...
            normalise_rew=self.normalise_rewards,
            device=device,
            # avoids overwriting training temp files - can be disastrous!
        ...
        while test_envs.get_env_attr('cur_step') < test_envs.get_env_attr('steps_limit'):
            ...
            task_gating_values = torch.stack(gating_values).cpu()
            self.logger.add_tensorboard(
                f'{eval_run}/episode_rewards', task_rewards.mean(), frames)
            self.logger.add_tensorboard(
